...
- [ ] Run `Tools > Clash of Pantheons > Bind Production Card Views` in Unity, then verify all five cards' hover/tap selection, artwork, status, tier, affordability, purchase, and mythic parchment behavior. `ProductionCardPresenter` now consumes serialized `ProductionCardView` references rather than exact hierarchy names, and the HUD builder authors those bindings for regenerated cards. Convert the cards to a shared prefab only after this binding migration is verified.
