...
- Continue behavior-preserving tests while splitting the oversized `BattleEconomyUI` and `GameManager` responsibilities. Timeout resolution, player-relative result/countdown text, production-tier transitions, and the accepted star-stat curve now live in the Core assembly, with all 38 Edit Mode cases passing in Unity on 2026-07-18. Runtime mythic-picker, production-card, selected-role, and shared mythic artwork presentation are separated and verified. Production tiers, selected mythics, timers, and ready-role indices now live in `ProductionStateController`; unlock, upgrade, cadence, mythic, AI, cap-resume, match-end, and restart behavior were verified in Play Mode. Extract production spawn scheduling next while preserving faction lookup, prefab instantiation, atomic purchases, and exact loss accounting.
