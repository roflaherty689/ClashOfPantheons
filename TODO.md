...
- [ ] Run `Tools > Clash of Pantheons > Normalize Battle Scene UI Hierarchy` in the open Unity Editor. The guarded migration removes only the isolated `Canvas/Canvas/VictoryText` legacy branch and restores the active `Battle UI` RectTransform scale to one; the HUD builder now also authors that scale explicitly. Reopen the scene and verify pointer input, result overlay, and representative aspect ratios before marking complete.
- [ ] Run `Tools > Clash of Pantheons > Convert Gold Veins to Shared Prefab` in Unity. The guarded migration creates `Assets/Prefabs/Resources/GoldVein.prefab`, retains side-specific root transforms, keeps `MinePoint` internal to each instance, reconnects both `WorkerManager` references, and validates the result before saving. Verify both worker loops before marking the gold-vein portion complete. Production-card prefab work remains separate until its view bindings are serialized rather than name-driven.
