...
- Continue behavior-preserving tests while splitting the oversized `BattleEconomyUI` and `GameManager` responsibilities. Timeout resolution, player-relative result text, production-tier transitions, and the accepted star-stat curve now live in the Core assembly, with all 32 Edit Mode cases passing in Unity on 2026-07-18. Prefer the next seams around production presentation and the mythic picker; preserve serialized names, initialization order, atomic mythic purchase, timer resets, and exact loss accounting.
