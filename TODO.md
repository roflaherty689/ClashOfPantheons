...
- Continue behavior-preserving tests while splitting the oversized `BattleEconomyUI` and `GameManager` responsibilities. Timeout resolution and player-relative result text now live in the Core assembly, with all 15 Edit Mode cases passing in Unity on 2026-07-18. Prefer the next seams around production transactions/presentation and the mythic picker; preserve serialized names, initialization order, atomic mythic purchase, and exact loss accounting.
